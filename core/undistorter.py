...
def get_transform(corner_pin: List[Tuple[int, int]], width: int=1280, height: int=720) -> Tuple:
    aspect_ratio = get_aspect_ratio(corner_pin, width, height)
    dimensions = get_dimensions(corner_pin, aspect_ratio)

    scoreboard_corners = list()
    scoreboard_corners.append((0, 0))
    scoreboard_corners.append((dimensions[0] - 1, 0))
    scoreboard_corners.append((dimensions[0] - 1, dimensions[1] - 1))
    scoreboard_corners.append((0, dimensions[1] - 1))

    src = np.array(corner_pin, dtype = "float32")
    dest = np.array(scoreboard_corners, dtype = "float32")
    transform_matrix = cv2.getPerspectiveTransform(src, dest)
